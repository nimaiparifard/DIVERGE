# Experiment: Line plot for entropy top-k different ranges and node classification metrics
# 
# This experiment:
...
#    - Removes that percentage of edges with highest entropy uncertainty
...
RESULTS_DIR = os.path.join(STRUCTURE_DIR, 'results', 'line_plot_entropy_topk')
...
def sweep_topk_and_train(
    ...
    """
    print(f"\n{'='*80}")
    print(f"SWEEPING TOP-K PERCENTAGES (ENTROPY)")
    print(f"{'='*80}")
    ...
    for topk_pct in topk_percentages:
        ...
        # Remove top-k% of edges with highest entropy
...
def plot_line_chart(
    ...
    ax1.set_title(f'Node Classification: Baseline vs After Edge Removal (Entropy Top-k) — {dataset_name}', 
    ...
    save_path = os.path.join(save_dir, f'line_plot_{metric_clean}_entropy_topk_{dataset_name}.png')
    ...
    ax.set_title(f'Node Classification Metrics vs Edge Removal (Entropy Top-k) — {dataset_name}', 
                 fontsize=13, fontweight='bold')
    ...
    plt.tight_layout()
    save_path2 = os.path.join(save_dir, f'line_plot_combined_{metric_clean}_entropy_topk_{dataset_name}.png')
...
def main():
    parser = argparse.ArgumentParser(
        description='Line plot experiment: entropy top-k vs node classification metrics'
    )
    ...
    print('=' * 80)
    print('LINE PLOT EXPERIMENT: ENTROPY TOP-K vs NODE CLASSIFICATION')
    print('=' * 80)
