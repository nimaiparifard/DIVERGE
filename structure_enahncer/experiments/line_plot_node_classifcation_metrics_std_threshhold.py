# Experiment: Line plot for std threshold different ranges and node classification metrics
# 
# This experiment:
# 1. Trains K edge predictors to compute std uncertainty scores for all edges
# 2. Sweeps through different threshold values (e.g., 0.01, 0.02, 0.03, ..., 0.10)
# 3. For each threshold:
#    - Removes edges with std uncertainty > threshold
...
#
# X-axis: Threshold value (std uncertainty)
...
RESULTS_DIR = os.path.join(STRUCTURE_DIR, 'results', 'line_plot_std_threshold')
...
def sweep_threshold_and_train(
    ...
    Args:
        ...
        thresholds: List of threshold values to try (e.g., [0.01, 0.02, 0.03])
    ...
    """
    print(f"\n{'='*80}")
    print(f"SWEEPING STD THRESHOLD VALUES")
    print(f"{'='*80}")
    ...
    for threshold in thresholds:
        ...
        # Remove edges with std > threshold
...
def plot_line_chart(
    ...
    ax1.set_title(f'Node Classification: Baseline vs After Edge Removal (Std Threshold) — {dataset_name}', 
    ...
    ax2.set_xlabel('Std Threshold (Uncertainty)', fontsize=12)
    ...
    save_path = os.path.join(save_dir, f'line_plot_{metric_clean}_std_threshold_{dataset_name}.png')
    ...
    ax.set_xlabel('Std Threshold (Uncertainty)', fontsize=12)
    ax.set_ylabel(f'{metric.replace("_", " ").title()}', fontsize=12)
    ax.set_title(f'Node Classification Metrics vs Std Threshold — {dataset_name}', 
                 fontsize=13, fontweight='bold')
    ...
    plt.tight_layout()
    save_path2 = os.path.join(save_dir, f'line_plot_combined_{metric_clean}_std_threshold_{dataset_name}.png')
...
def main():
    parser = argparse.ArgumentParser(
        description='Line plot experiment: std threshold vs node classification metrics'
    )
    ...
    parser.add_argument('--threshold_start', type=float, default=0.01,
                        help='Starting threshold value (default: 0.01)')
    parser.add_argument('--threshold_end', type=float, default=0.80,
                        help='Ending threshold value (default: 0.20)')
    ...
    print('=' * 80)
    print('LINE PLOT EXPERIMENT: STD THRESHOLD vs NODE CLASSIFICATION')
    print('=' * 80)
