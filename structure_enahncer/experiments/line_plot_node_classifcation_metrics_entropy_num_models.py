# Experiment: Analyze influence of number of models (K) on node classification metrics using entropy uncertainty
# 
# This experiment:
# 1. Sweeps through different numbers of edge predictor models (K): 2, 5, 7, 10, 13, 15, 20, 30, 40, 50
# 2. For each K:
#    - Trains K edge predictors to compute entropy uncertainty scores for all edges
...
RESULTS_DIR = os.path.join(STRUCTURE_DIR, 'results', 'line_plot_entropy_num_models')
...
def sweep_num_models_and_train(
    ...
    for K in num_models_list:
        ...
        result_entry = {
            ...
            'entropy_mean': entropy_scores.mean().item(),
            'entropy_std': entropy_scores.std().item(),
        }
        ...
        print(f"K={K}: {metric} = {modified_metric:.4f}, "
              ...
              f"Entropy mean = {result_entry['entropy_mean']:.4f}")
...
def plot_line_chart(
    ...
    entropy_means = [r['entropy_mean'] for r in num_models_results]
    ...
    ax1.set_title(f'Node Classification vs Number of Models K (Entropy, {strategy_label}) — {dataset_name}', 
    ...
    save_path = os.path.join(save_dir, f'line_plot_{metric_clean}_entropy_num_models_{strategy_clean}_{dataset_name}.png')
    ...
    ax.set_title(f'Node Classification Metrics vs Number of Models K (Entropy) — {dataset_name}', 
                 fontsize=13, fontweight='bold')
    ...
    plt.tight_layout()
    save_path2 = os.path.join(save_dir, f'line_plot_combined_{metric_clean}_entropy_num_models_{dataset_name}.png')
    plt.savefig(save_path2, dpi=150, bbox_inches='tight')
    print(f"[OK] Saved combined plot: {save_path2}")
    plt.close()
    
    # --- Additional: Plot entropy statistics vs K ---
    fig3, ax = plt.subplots(figsize=(10, 5))
    ax.plot(
        num_models_list,
        entropy_means,
        '-^',
        label='Mean Entropy Uncertainty',
        linewidth=2,
        color='purple',
        markersize=6
    )
    ax.set_xlabel('Number of Models (K)', fontsize=12)
    ax.set_ylabel('Mean Entropy Uncertainty', fontsize=12)
    ax.set_title(f'Entropy Uncertainty vs Number of Models K — {dataset_name}', 
                 fontsize=13, fontweight='bold')
    ax.legend(loc='best', fontsize=10)
    ax.grid(True, alpha=0.3)
    ax.set_xlim([min(num_models_list) - 1, max(num_models_list) + 1])
    plt.tight_layout()
    save_path3 = os.path.join(save_dir, f'entropy_uncertainty_vs_K_{dataset_name}.png')
    plt.savefig(save_path3, dpi=150, bbox_inches='tight')
    print(f"[OK] Saved entropy uncertainty plot: {save_path3}")
    plt.close()


def main():
    parser = argparse.ArgumentParser(
        description='Line plot experiment: number of models (K) vs node classification metrics (entropy)'
    )
    ...
    parser.add_argument('--removal_strategy', type=str, default='threshold',
                        choices=['top_k', 'threshold'],
                        help='Edge removal strategy (default: top_k)')
    parser.add_argument('--removal_threshold', type=float, default=0.35,
    ...
    print('=' * 80)
    print('LINE PLOT EXPERIMENT: NUMBER OF MODELS (K) vs NODE CLASSIFICATION (ENTROPY)')
    print('=' * 80)
    ...
    for r in num_models_results:
        print(f"  K={r['num_models']:2d}: {r['metric_value']:.4f} "
              ...
              f"entropy_mean: {r['entropy_mean']:.4f})")
