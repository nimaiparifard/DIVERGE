...
def main():
    parser = argparse.ArgumentParser(
        description='Ensemble edge predictor with std uncertainty: tune hyperparameters, find top 5, run ensemble'
    )
    
    parser.add_argument('--dataset_name', type=str, default='citeseer', help='Dataset name')
    ...
    parser.add_argument('--threshold_values', type=float, nargs='+',
                        default=[0.05, 0.10, 0.15, 0.20, 0.25, 0.30, 0.4],
    ...
    parser.add_argument('--save_dir', type=str, default='results/ensemble_edge_predictor_std',
                        help='Directory to save results')
    
    args = parser.parse_args()
    # list from 0.0 to 50 with 1 step
    topk_percentage = np.arange(0.0, 50.0, 1.0).tolist()
    # threshold list from 0.0 to 0.4 with 0.01 step
    threshold_values = np.arange(0.0, 0.4, 0.01).tolist()
    os.makedirs(args.save_dir, exist_ok=True)
    
    print('=' * 80)
    print('ENSEMBLE EDGE PREDICTOR WITH STD UNCERTAINTY')
    print('=' * 80)
    ...
    print(f"{'='*80}")
    topk_configs, baseline_result = tune_hyperparameters_std(
        gnn_cfg=gnn_cfg,
        embedding=embedding,
    ...
    reporter.report_title("ENSEMBLE EDGE PREDICTOR WITH STD UNCERTAINTY")
