...
from torch_geometric.nn import GCNConv, GATConv, TransformerConv, SAGEConv, GINConv, DenseGATConv
...
class GNNEncoder(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, n_layers=2, gnn_type="GCN", dropout=0.0, use_softmax=0, 
                 batch_norm=0, residual_conn=0, jump_knowledge=0, batch_norm_type="batch"):
        super().__init__()
        ...
        elif batch_norm_type == "id":
            self.bns = nn.ModuleList([nn.Identity(hidden_dim) for _ in range(n_layers-1)])
        elif batch_norm_type == "layer":
            self.bns = nn.ModuleList([nn.LayerNorm(hidden_dim) for _ in range(n_layers-1)])
